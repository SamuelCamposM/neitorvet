...
import 'package:neitorvet/features/products/presentation/provider/form/product_form_provider.dart';
import 'package:neitorvet/features/products/presentation/provider/product_provider.dart';
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return GestureDetector(
      ...
      child: Scaffold(
          ...
          actions: [
            IconButton(
                onPressed: () async {
                  ...
                  if (photPath == null) return;
                  ref
                      .read(productFormProvider(productState.product!).notifier)
                      .updateProductImage(photPath);
                },
                icon: const Icon(Icons.photo_library_outlined)),
            IconButton(
                onPressed: () async {
                  final photPath = await CameraGalleryServiceImpl().takePhoto();
                  if (photPath == null) return;
                  ref
                      .read(productFormProvider(productState.product!).notifier)
                      .updateProductImage(photPath);
        ...
        floatingActionButton: productState.isLoading
            ? null
            : _FloatingButton(
                product: productState.product!,
              ),
...
class _FloatingButton extends ConsumerWidget {
  ...
  const _FloatingButton({required this.product});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          const Text('Generales', textAlign: TextAlign.center),
...
class _GenderSelector extends StatelessWidget {
  ...
  final void Function(String selectedSizes)? onGenderChanged;
    ...
    return PageView(
      ...
      children: images.map((image) {
        ...
        return Padding(
          ...
          child: ClipRRect(
            borderRadius: const BorderRadius.all(Radius.circular(20)),
            child: FadeInImage(fit: BoxFit.cover, image: imageProvider, placeholder: const AssetImage('assets/loaders/bottle-loader.gif'),),
          ),
        );
      }).toList(),
    );
